...
<div>
    ...
    @endphp
    <a href="{{route('pemohon.create')}}"><x-adminlte-button class="mb-2 btn-flat" label="Tambah" theme="success" icon="fas fa-lg fa-user-plus" /></a>
    ...
    <x-adminlte-datatable id="table1" :heads="$heads">
        ...
        <tr>
            ...
            <td>
                <nobr>
                    <a href="{{route('pemohon.edit', $row['id'])}}">
                        <button class="btn btn-xs btn-default text-primary mx-1 shadow" title="Edit">
                            <i class="fa fa-lg fa-fw fa-pen"></i>
                        </button>
                    </a>
                    <a href="{{route('pemohon.show', $row['id'])}}">
                    ...
                    <x-adminlte-modal id="modalDelete{{$row['id']}}" title="Hapus Akun ?" size="sm" theme="teal" icon="fas fa-bell" v-centered static-backdrop scrollable>
                        <x-slot name="footerSlot">
                            <form action="{{route('pemohon.destroy', $row['id'])}}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
